// VML DECK LAYOUTS v1.9
// v1.7: relative positioning. v1.8: valign in rows. v1.9: expose resolveLayouts globally
(function(){
'use strict';
...
if(hasFootnote){var fnY=2.2+nRows*(cardH+gap);els.push({type:'t',text:s.footnote,x:.5,y:fnY,w:11,h:.3,font:'B',size:11,color:'muted'});}
return els;}
...
els.push({type:'t',text:s.title||'',x:.5,y:1.05,w:11,h:.5,font:'H',size:36,color:'title'});
if(s.subtitle)els.push({type:'t',text:s.subtitle,x:.5,y:1.65,w:10,h:.3,font:'B',size:12,color:'body'});
var gap=0.2;var totalH=6.2-2.2;var rawH=(totalH-(nRows-1)*gap)/nRows;var cardH=Math.min(rawH,1.6);var usedH=nRows*cardH+(nRows-1)*gap;var startY=2.2+(totalH-usedH)/2;
...
if(labelW>=1.8){var labelX=ix+valW+0.1;els.push({type:'t',text:item.value||'',x:ix,y:cy+0.2,w:valW,h:.6,font:'H',size:36,color:'accent'});els.push({type:'t',text:(item.label||'').toUpperCase(),x:labelX,y:cy+0.2,w:labelW,h:.3,font:'H',size:13,color:'title'});els.push({type:'t',text:item.text||'',x:labelX,y:cy+0.6,w:labelW,h:cardH-0.9,font:'B',size:11,color:'body'});
}else{els.push({type:'t',text:item.value||'',x:ix,y:cy+0.1,w:tw,h:.45,font:'H',size:36,color:'accent'});els.push({type:'t',text:(item.label||'').toUpperCase(),x:ix,y:cy+0.6,w:tw,h:.25,font:'H',size:13,color:'title'});els.push({type:'d',x:ix,y:cy+0.9,w:dw,color:'ltGray'});els.push({type:'t',text:item.text||'',x:ix,y:cy+1.0,w:tw,h:cardH-1.15,font:'B',size:11,color:'body'});}});
return els;}
...
if(numbered){
els.push({type:'t',text:num,x:.7,y:ry,w:0.6,h:rowH,font:'H',size:rowH<0.7?18:28,color:'accent',valign:'middle'});
els.push({type:'t',text:item.title||'',x:1.4,y:ry,w:rowH<0.7?2.5:3.5,h:rowH,font:'H',size:rowH<0.7?12:13,color:'title',valign:'middle'});
els.push({type:'t',text:item.text||'',x:rowH<0.7?4.0:5.5,y:ry,w:rowH<0.7?8.0:7.0,h:rowH,font:'B',size:11,color:'body',valign:'middle'});
}else{
...
return els;}
// v1.9: expose resolveLayouts for dlP() in image mode
