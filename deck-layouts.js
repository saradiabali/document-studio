...
if(compact){
if(item.icon){
els.push({type:'i',icon:item.icon,x:ix,y:ny+0.02,w:.35,h:.35});
...
els.push({type:'d',x:ix,y:ny,w:dw,color:'ltGray'});
ny+=0.15;
}else{
if(item.icon){els.push({type:'i',icon:item.icon,x:ix,y:ny+0.05,w:.5,h:.5});ny+=0.75;}
els.push({type:'t',text:item.title||'',x:ix,y:ny,w:tw,h:.55,font:'H',size:15,color:'title'});
ny+=0.6;
...
if(hasFootnote){
var fnY=2.2+nRows*(cardH+gap);
els.push({type:'t',text:s.footnote,x:.5,y:fnY,w:11,h:.3,font:'B',size:11,color:'muted'});
}
return els;
}
...
var totalH=6.2-2.2;
var rawH=(totalH-(nRows-1)*gap)/nRows;
var cardH=Math.min(rawH,1.6);
var usedH=nRows*cardH+(nRows-1)*gap;
var startY=2.2+(totalH-usedH)/2;
items.forEach(function(item,i){
var col=i%cols;
var row=Math.floor(i/cols);
var cx=grid[col].x;
...
var dw=cw-0.4;
els.push({type:'s',x:cx,y:cy,w:cw,h:cardH,fill:'cardBg',border:'ltGray',bw:1});
var valLen=(item.value||'').length;
var valW=Math.max(1.5,valLen*0.35);
var labelW=tw-valW-0.1;
if(labelW>=1.8){
var labelX=ix+valW+0.1;
els.push({type:'t',text:item.value||'',x:ix,y:cy+0.2,w:valW,h:.6,font:'H',size:36,color:'accent'});
els.push({type:'t',text:(item.label||'').toUpperCase(),x:labelX,y:cy+0.2,w:labelW,h:.3,font:'H',size:13,color:'title'});
els.push({type:'t',text:item.text||'',x:labelX,y:cy+0.6,w:labelW,h:cardH-0.9,font:'B',size:11,color:'body'});
}else{
els.push({type:'t',text:item.value||'',x:ix,y:cy+0.1,w:tw,h:.45,font:'H',size:36,color:'accent'});
els.push({type:'t',text:(item.label||'').toUpperCase(),x:ix,y:cy+0.6,w:tw,h:.25,font:'H',size:13,color:'title'});
els.push({type:'d',x:ix,y:cy+0.9,w:dw,color:'ltGray'});
els.push({type:'t',text:item.text||'',x:ix,y:cy+1.0,w:tw,h:cardH-1.15,font:'B',size:11,color:'body'});
}
});
return els;
}
...
if(tight){
if(numbered){
els.push({type:'t',text:num,x:.7,y:ry+0.08,w:0.6,h:.4,font:'H',size:18,color:'accent'});
els.push({type:'t',text:item.title||'',x:1.4,y:ry+0.08,w:2.5,h:.4,font:'H',size:12,color:'title'});
els.push({type:'t',text:item.text||'',x:4.0,y:ry+0.08,w:8.0,h:.4,font:'B',size:11,color:'body'});
}else{
els.push({type:'t',text:item.title||'',x:.7,y:ry+0.08,w:3.0,h:.4,font:'H',size:12,color:'title'});
els.push({type:'t',text:item.text||'',x:3.8,y:ry+0.08,w:8.2,h:.4,font:'B',size:11,color:'body'});
}
}else{
if(numbered){
els.push({type:'t',text:num,x:.7,y:ry+0.1,w:1.0,h:.35,font:'H',size:28,color:'accent'});
els.push({type:'t',text:item.title||'',x:1.8,y:ry+0.1,w:3.5,h:.35,font:'H',size:13,color:'title'});
els.push({type:'t',text:item.text||'',x:1.8,y:ry+0.45,w:9.8,h:rowH-0.55,font:'B',size:11,color:'body'});
}else{
els.push({type:'t',text:item.title||'',x:.7,y:ry+0.1,w:11.0,h:.35,font:'H',size:13,color:'title'});
els.push({type:'t',text:item.text||'',x:.7,y:ry+0.45,w:11.0,h:rowH-0.55,font:'B',size:11,color:'body'});
}
}
});
